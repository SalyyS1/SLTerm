...
func init() {
	...
	tabIndicatorCmd.Flags().StringVar(&tabIndicatorTabId, "tabid", "", "tab id (defaults to SLTERM_TABID)")
...
func tabIndicatorRun(cmd *cobra.Command, args []string) (rtnErr error) {
	...
	if tabId == "" {
		tabId = os.Getenv("SLTERM_TABID")
	}
	if tabId == "" {
		return fmt.Errorf("no tab id specified (use --tabid or set SLTERM_TABID)")
	}
