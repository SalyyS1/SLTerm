...
var (
	rootCmd = &cobra.Command{
		Use:          "wsh",
		Short:        "CLI tool to control SL Terminal",
		Long:         `wsh is a small utility that lets you do cool things with SL Terminal, right from the command line`,
		SilenceUsage: true,
	}
)
...
func preRunSetupRpcClient(cmd *cobra.Command, args []string) error {
	...
	if jwtToken == "" {
		return fmt.Errorf("wsh must be run inside a Wave-managed SSH session (SLTERM_JWT not found)")
	}
...
func setupRpcClient(serverImpl wshutil.ServerImpl, jwtToken string) error {
	...
	blockId := os.Getenv("SLTERM_BLOCKID")
	if blockId != "" {
...
func resolveSimpleId(id string) (*waveobj.ORef, error) {
	...
	blockId := os.Getenv("SLTERM_BLOCKID")
	if blockId == "" {
		return nil, fmt.Errorf("no SLTERM_BLOCKID env var set")
	}
