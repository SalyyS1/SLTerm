...
func WaveshellLocalEnvVars(termType string) map[string]string {
	...
	rtn["TERM_PROGRAM"] = "slterm"
	rtn["SLTERM"], _ = os.Executable()
	rtn["SLTERM_VERSION"] = wavebase.WaveVersion
	rtn["SLTERM_WSHBINDIR"] = filepath.Join(wavebase.GetWaveDataDir(), WaveHomeBinDir)
	return rtn
}
