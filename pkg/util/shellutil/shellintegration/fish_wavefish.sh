...
wsh token "$SLTERM_SWAPTOKEN" fish 2>/dev/null | source
set -e SLTERM_SWAPTOKEN

# Load Wave completions
wsh completion fish | source

set -g _SLTERM_SI_FIRSTPROMPT 1

# shell integration
function _slterm_si_blocked
...
function _slterm_si_osc7
    _slterm_si_blocked; and return
    # Use fish-native URL encoding
    set -l encoded_pwd (string escape --style=url -- "$PWD")
    printf '\033]7;file://localhost%s\007' $encoded_pwd
end

function _slterm_si_prompt --on-event fish_prompt
    set -l _slterm_si_status $status
    _slterm_si_blocked; and return
    if test $_SLTERM_SI_FIRSTPROMPT -eq 1
    ...
    set -g _SLTERM_SI_FIRSTPROMPT 0
end

function _slterm_si_preexec --on-event fish_preexec
    _slterm_si_blocked; and return
...
function _slterm_si_chpwd --on-variable PWD
    _slterm_si_osc7
end