...
source <(wsh token "$SLTERM_SWAPTOKEN" zsh 2>/dev/null)
...
# fix history (macos)
if [[ "$HISTFILE" == "$SLTERM_ZDOTDIR/.zsh_history" ]]; then
  HISTFILE="$HOME/.zsh_history"
fi

typeset -g _SLTERM_SI_FIRSTPRECMD=1
...
_slterm_si_precmd() {
  ...
  if (( !_SLTERM_SI_FIRSTPRECMD )); then
...
typeset -g SLTERM_SI_INPUTEMPTY=1

_slterm_si_inputempty() {
  _slterm_si_blocked && return
  ...
  if (( current_empty != SLTERM_SI_INPUTEMPTY )); then
    SLTERM_SI_INPUTEMPTY=$current_empty
...
if (( $+functions[add-zle-hook-widget] )); then
  add-zle-hook-widget zle-line-init _slterm_si_inputempty
  add-zle-hook-widget zle-line-pre-redraw _slterm_si_inputempty
fi

autoload -U add-zsh-hook
add-zsh-hook precmd  _slterm_si_precmd
add-zsh-hook preexec _slterm_si_preexec
add-zsh-hook chpwd   _slterm_si_osc7