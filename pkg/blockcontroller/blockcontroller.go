...
func makeSwapToken(ctx context.Context, logCtx context.Context, blockId string, blockMeta waveobj.MetaMapType, remoteName string, shellType string) *shellutil.TokenSwapEntry {
	...
	token.Env["TERM_PROGRAM"] = "slterm"
	token.Env["SLTERM_BLOCKID"] = blockId
	token.Env["SLTERM_VERSION"] = wavebase.WaveVersion
	token.Env["SLTERM"] = "1"
	tabId, err := wstore.DBFindTabForBlockId(ctx, blockId)
	if err != nil {
		log.Printf("error finding tab for block: %v\n", err)
	} else {
		token.Env["SLTERM_TABID"] = tabId
	}
	if tabId != "" {
		...
		if err != nil {
			...
			token.Env["SLTERM_WORKSPACEID"] = wsId
		}
	}
	token.Env["SLTERM_CLIENTID"] = wstore.GetClientId()
	token.Env["SLTERM_CONN"] = remoteName
