...
// On Windows, it will store to %LOCALAPPDATA%/slterm/electron
app.setName("slterm/electron");
...
const waveDirNamePrefix = "slterm";
const waveDirNameSuffix = isDev ? "dev" : "";
const waveDirName = `${waveDirNamePrefix}${waveDirNameSuffix ? `-${waveDirNameSuffix}` : ""}`;

const paths = envPaths("slterm", { suffix: waveDirNameSuffix });

app.setName(isDev ? "SLTerm (Dev)" : "SLTerm");
const unamePlatform = process.platform;
const unameArch: string = process.arch;
keyutil.setKeyUtilPlatform(unamePlatform);

const WaveConfigHomeVarName = "SLTERM_CONFIG_HOME";
const WaveDataHomeVarName = "SLTERM_DATA_HOME";
const WaveHomeVarName = "SLTERM_HOME";
...
export function checkIfRunningUnderARM64Translation(fullConfig: FullConfigType) {
    if (!fullConfig.settings["app:dismissarchitecturewarning"] && app.runningUnderARM64Translation) {
        ...
        const dialogOpts: Electron.MessageBoxOptions = {
            ...
            title: "SLTerm has detected a performance issue",
            message: `SLTerm is running in ARM64 translation mode which may impact performance.\n\nRecommendation: Download the native ARM64 version for optimal performance.`,
        };
        ...
        if (choice === 1) {
            ...
            fireAndForget(() =>
                shell.openExternal(
                    "https://github.com/SalyyS1/SLTerm/faq#why-does-wave-warn-me-about-arm64-translation-when-it-launches"
                )
            );
...
/**
 * Gets the path to the old Wave home directory (defaults to `~/.slterm`).
...
function getElectronAppBasePath(): string {
    // import.meta.dirname in dev points to slterm/dist/main
