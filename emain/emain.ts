...
console.log(
    sprintf(
        "slterm-app starting, data_dir=%s, config_dir=%s electronpath=%s gopath=%s arch=%s/%s electron=%s",
        waveDataDir,
        waveConfigDir,
        getElectronAppBasePath(),
        getElectronAppUnpackedBasePath(),
...
if (isDev) {
    console.log("slterm-app SLTERM_DEV set");
}
...
electronApp.on("before-quit", (e) => {
    ...
    if (
        ...
        !getIsWaveSrvDead() &&
        !process.env.SLTERM_NOCONFIRMQUIT
    ) {
        e.preventDefault();
        const choice = electron.dialog.showMessageBoxSync(null, {
            ...
            buttons: ["Cancel", "Quit"],
            title: "Confirm Quit",
            message: "Are you sure you want to quit SLTerm?",
            defaultId: 0,
            cancelId: 0,
        });
        if (choice === 0) {
...
async function appMain() {
    ...
    if (!instanceLock) {
        console.log("slterm-app could not get single-instance-lock, shutting down");
        setUserConfirmedQuit(true);
        electronApp.quit();
        return;
    }
